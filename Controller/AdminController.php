...
if (isset($_POST['action']) && $_POST['action'] === 'add_citizen') {
    ...
    if ($name && $email && $password) {
        $result = createUser($name, $email, $phone, $password, 'citizen', $address, $nid);
...
if (isset($_POST['action']) && $_POST['action'] === 'add_authority') {
    ...
    if ($name && $email && $phone && $password) {
        $result = createUser($name, $email, $phone, $password, 'authority', $address, $nid);
...
// Handle unblock user operations
...
if (isset($_POST['action']) && $_POST['action'] === 'add_announcement') {
    ...
    if ($title && $message) {
        $result = createAnnouncement($title, $message, $_SESSION['id']);
        
        if ($isAjax) {
            if ($result) {
                sendJsonResponse(true, "Announcement created successfully!", ['id' => $result]);
            } else {
                sendJsonResponse(false, "Failed to create announcement");
            }
        }
        ...
        if ($isAjax) {
            sendJsonResponse(false, "Title and message are required");
        }
...
if (isset($_POST['action']) && $_POST['action'] === 'add_incident') {
    ...
    if (isset($_FILES['issue_image']) && $_FILES['issue_image']['name'] != '') {
        $originalName = str_replace(" ", "_", $_FILES['issue_image']['name']);
    ...
    if ($title && $description && $location) {
        // Use admin's ID as the creator
        $result = insertIssue($_SESSION['id'], $title, $description, $location, $image);
        
        if ($isAjax) {
            if ($result) {
                sendJsonResponse(true, "Incident created successfully!");
            } else {
                sendJsonResponse(false, "Failed to create incident");
            }
        }
        
        if ($result) {
            $_SESSION['msg'] = "Incident created successfully!";
        } else {
            $_SESSION['msg'] = "Failed to create incident";
        }
    } else {
        if ($isAjax) {
            sendJsonResponse(false, "Title, description and location are required");
        }
        $_SESSION['msg'] = "Title, description and location are required";
    }
